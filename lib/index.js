...
const { ACTIONS, STEPS, OPTIONS } = require("./constants");
...
module.exports = async () => {
  try {
    ...
    if (action === ACTIONS.NEW) {
      ...
      projectDir = path.join(dir, projectName);
      console.log("Project Created!");
      fs.writeFileSync(
        `${path.join(cwd, projectDir)}/upgradeHelper.json`,
    ...
    const { installed = [], packages = {} } = require(path.join(cwd, projectDir) + "/upgradeHelper.json");
